...
| Repository or storage for results | Store predictions, metrics, and analytical outputs in MinIO Gold; evaluate Elasticsearch as a downstream store. | Gold predictions (638,176 rows, `s3a://bigdata/gold/predictions`) and final metrics manifest (`s3a://bigdata/gold/metrics/final_test_evaluation_report.json`) | VALIDATED |
| Results and insights | Analyze forecast accuracy by horizon and the incremental value of weather. | Phase 5B validation/test MAE and RMSE for three non-ML baselines; Phase 5C measured weather A/B deltas (negative at every horizon); Phase 5D frozen final TEST metrics and weather deltas in `docs/final_test_evaluation_validation.md` | VALIDATED |
| AI capability on project data | Train and evaluate demand-forecasting models using project-derived features. | Phase 5C selected Regularized Linear Regression by validation MAE; Phase 5D refit on train+validation and evaluated once on frozen test in `docs/final_test_evaluation_validation.md` and `data/silver/final_test_evaluation_report.json` | VALIDATED |
| Source code | Maintain reproducible pipeline, validation, modeling, and test code in Git. | Feasibility, Bronze, Silver ETL, modeling-feature, baseline, ML-selection, and final-evaluation packages, infrastructure smoke tests, and focused Spark tests | VALIDATED |
...
| README with run instructions | Expand the README with prerequisites, setup, execution, validation, and troubleshooting steps. | Polished `README.md`: summary, architecture, results table, per-phase focused-test and full-run commands, validation summary, limitations | VALIDATED |
| 1–2 page design document | Summarize architecture, data flow, modeling, validation, and trade-offs. | `docs/design_document.md` | VALIDATED |
| Architecture diagram | Diagram sources, processing, storage layers, modeling, and outputs. | `docs/architecture_diagram.md` (version-controlled Mermaid source; no local renderer available, so no SVG/PNG export is committed) | VALIDATED |
| Data-flow and technology explanation | Explain each component's role and why it is necessary. | README "Architecture overview"/"Technologies" sections and `docs/design_document.md` | VALIDATED |
...
| Results, challenges, and trade-offs explanation | Report only observed results and explain limitations and design compromises. | `docs/design_document.md` and README "Limitations" section; presentation still pending | IN PROGRESS |
